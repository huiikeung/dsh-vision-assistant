...
.map((e) => ({
   ...
   *  旧 namespace 未注册时 settingsService.get() 返回 undefined——此时用宿主的
   *  register() 把它临时注册进来（复用宿主的 YAML 解析与 schema 校验）再读。
  ...
  async function migrateLegacyNamespaceOnce(attempt = 1) {
    ...
    try {
      ...
      if (legacy === void 0 || legacy === null || typeof legacy !== 'object') {
        ...
        } catch { /* 已被注册（不可能）或宿主限制：放弃迁移 */ }
      }
      if (legacy === void 0 || legacy === null || typeof legacy !== 'object') return;
      ...
      const next = {
        ...current,
        visionModels: mergeById(current.visionModels ?? [], legacyModels),
    ...
    wctx.effect(() => wctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          // 官方目录补充：运行时 listModels 只覆盖「配置了密钥且网关如实申报模态」的
          // 渠道；很多供应商（本地网关/目录滞后）不报 inputModalities，导致一键导入
          // 只看到少数几个供应商。这里用 pi-ai 官方静态目录的 input 声明兜底——但只对
          // 已配置密钥的内置供应商生效（没 key 的列出来也只是死条目），标记 catalog:true。
          try {
            const credentials = ctx.get('credentials');
            const builtinCatalog = await piAiCatalog;
            if (builtinCatalog !== void 0 && typeof builtinCatalog.getBuiltinModels === 'function') {
              const imported = new Set((options().visionModels ?? [])
                .map((e) => (e && e.provider ? `${e.provider}/${e.model}` : '')).filter(Boolean));
              const grouped = new Set(systemGroups.map((g) => g.provider));
              const hasKey = async (providerId) => {
                const ref = deriveKeyRef(providerId);
                const envKeys = typeof process !== 'undefined' && process.env ? splitKeys(process.env[ref]) : [];
              ...
              for (const providerId of builtinCatalog.getBuiltinProviders()) {
                if (grouped.has(providerId) && !await hasKey(providerId)) continue;
                let vision = [];
                try {
                  vision = builtinCatalog.getBuiltinModels(providerId)
                    .filter((m) => Array.isArray(m?.input) && m.input.includes('image'))
                    .map((m) => ({ id: m.id, name: m.name || m.id }));
                } catch { continue; }
                if (vision.length === 0) continue;
                const fresh = vision.filter((m) => !imported.has(`${providerId}/${m.id}`));
                if (fresh.length === 0) continue;
                const existing = systemGroups.find((g) => g.provider === providerId);
                if (existing !== void 0) {
                  const ids = new Set(existing.models.map((m) => m.id));
                  for (const m of fresh) if (!ids.has(m.id)) existing.models.push(m);
                } else {
                  if (!await hasKey(providerId)) continue;
                  systemGroups.push({
                    provider: providerId,
                    name: `${providerId}（官方目录）`,
                    catalog: true,
                    models: fresh,
                  });
                }
            ...
            ctx.logger.warn('vision-config: 官方目录候选补充失败（不影响运行时检测结果）', error);
          }
