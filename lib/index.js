...
.map((e) => ({
   ...
   *  本插件改用自己的 `vision-assistant` 后，首次启动把旧配置整体搬过来
   *  （幂等：新命名空间已有内容则跳过；旧命名空间只读，不删除、不改写）。 */
  async function migrateLegacyNamespaceOnce(attempt = 1) {
    ...
    try {
      const existing = settingsService.get(NS);
      if (existing !== void 0 && existing !== null
        && Array.isArray(existing.visionModels) && existing.visionModels.length > 0) return;
      const legacy = settingsService.get(LEGACY_NS);
      ...
      if (legacyModels.length === 0 && !(Array.isArray(legacy.freeChannels) && legacy.freeChannels.length > 0)) return;
      await settingsScope.replace({ ...options(), ...legacy });
      ctx.logger.info('vision-config: 已把旧 namespace vision-opencode 的配置迁移到 vision-assistant');
    } catch (error) {
