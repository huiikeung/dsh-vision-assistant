...
// 2. 注册 settings namespace `vision-opencode`（刻意沿用旧名，见下方 NS 注释）：
 ...
 *
 *  刻意沿用 `vision-opencode` 这个名字：dsh-vision-assistant 是 dsh-vision-opencode 的整合
 *  后继（同一份 MIT 代码为底座），沿用同一 namespace 意味着用户已有的 settings.yaml
 *  （选中的识图模型、已导入的 visionModels、autoConvert…）零迁移生效，历史会话里已沉淀的
 *  图片分析缓存（PERSISTED_PREFIX）也继续命中。代价是两者不能同时启用（namespace / tool /
 *  skill 都会撞名），由 profile 的 cordis.patch.yml 保证同一时间只有一行 enabled。 */
const NS = typeof dshSettings.settingsNamespace === 'function'
  ? dshSettings.settingsNamespace('vision-opencode')
  : 'vision-opencode';
