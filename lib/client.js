...
setModal({mode:"add", data:{id:"",provider:firstProvider,providerType:firstProvider,customProvider:"",model:"",name:"",description:"",baseUrl:"",requestFormat:"openai-completions"}});
			...
			// 用 MutationObserver 在设置面板渲染时把该条目的图标替换为眼睛图标
			// （与 dsh-auxiliary 的 DOM 注入同款手法；dataset 守卫防止无限重放）。
			var installVisionNavIcon = function(){
				...
				var eyeSvg = function(cls){
					return '<svg ' + (cls ? 'class="' + cls + '" ' : '') + 'width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">'
						+ '<path d="M1.5 8s2.5-4.5 6.5-4.5S14.5 8 14.5 8 12 12.5 8 12.5 1.5 8 1.5 8Z" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/>'
				...
				var apply = function(root){
					try {
						...
						for (var i = 0; i < buttons.length; i++) {
							var btn = buttons[i];
							if (btn.dataset && btn.dataset.visionNavIcon === "1") continue;
							...
							svg.outerHTML = eyeSvg(cls);
							btn.dataset.visionNavIcon = "1";
						}
			...
			exports.apply = function(ctx) {
				...
				try {
					if (typeof window !== "undefined") {
						if (window.__visionAssistantNavIconInstalled !== true) {
							window.__visionAssistantNavIconInstalled = true;
