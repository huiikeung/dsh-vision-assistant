...
// 获取可用模型：走插件后端代理 /vision-config/discover-models。
		...
		var FreeModelsModule = function(props){
			...
			var summaryHeader = function(metaText, extra){
				return createElement.apply(null, ["button",{type:"button", key:"head", className:"vmo-free-summary"+(open===true?" vmo-free-summary-open":""),
					...
					createElement("span",{className:"vmo-free-summary-title"}, "免费模型兜底链路"),
					createElement("span",{className:"vmo-free-summary-meta"}, metaText)
				].concat(extra || []));
			};
			if(loading && data === null){
				return createElement("section",{className:blockCls, "aria-label":"免费模型兜底链路"},
			...
			// 三层瀑布的第二层只展示「有 Key 的免费渠道」；免 Key 渠道属于第三层（独立模块）
			var keyedChain = (Array.isArray(data.chain) ? data.chain : []).filter(function(step){ return step.manual === true; });

			var chain = keyedChain;
			...
			var keyedChannels = (Array.isArray(data.channels) ? data.channels : []).filter(function(ch){ return ch && ch.keyless !== true; });
			var cards = keyedChannels.map(function(ch){
			...
			// 折叠头摘要：渠道数 / 链路状态（均只统计有 Key 的第二层渠道；免 Key 渠道在第三层模块）
			var chainCount = keyedChain.length;
			var enabledCount = keyedChannels.filter(function(c){ return c && c.enabled === true; }).length;
			var meta = keyedChannels.length + " 个渠道"
			...
			var groupHeader = createElement("button",{type:"button", key:"groupHead",
				...
				createElement("span",{className:"vmo-free-summary-chevron", "aria-hidden":"true"}, IconChevronDownOutline14 !== null ? createElement(IconChevronDownOutline14,{size:12}) : "›"),
				createElement("span",{className:"vmo-free-summary-title"}, "免费渠道"),
				createElement("span",{className:"vmo-free-summary-meta"},
					keyedChannels.length+" 个渠道 · 已启用 "+enabledCount));
			...
			var body = open !== true
				? [createElement("p",{className:"vmo-free-meta", key:"hint"},
					"点击展开：免费渠道（渠道级密钥）、免费链路降级顺序。免 Key 的 OVH 兜底在下一个独立模块。")]
				: [createElement("p",{className:C.intro, key:"intro"},
					"识图瀑布第二层：上半区模型都失败时，按下面的顺序逐个尝试这些免费渠道（渠道级填一次密钥，写进宿主凭据服务，不在模型条目里）。全部失败后再交给「免 Key 最终兜底」。"),
			...
			return createElement.apply(null, ["section",{className:blockCls, "aria-label":"免费模型兜底链路"},
				summaryHeader(meta)].concat(body));
		};

		// 三层瀑布的第三层：免 Key 匿名渠道（OVHcloud）最终兜底。
		// 开关持久化在 freeKeylessFallback；引擎侧固定排在整个免费链路的末尾。
		var FreeKeylessModule = function(props){
			var api = props && props.api ? props.api : null;
			var showToast = props && props.showToast ? props.showToast : function(){};
			var useState = react.useState;
			var useEffect = react.useEffect;
			var createElement = react.createElement;
			var dataState = useState(null);
			...
			var busy = busyState[0], setBusy = busyState[1];
			var load = function(){
				fetch("/vision-config/free-channels",{headers:{accept:"application/json"}})
					.then(function(r){ if(!r.ok) throw new Error("HTTP "+r.status); return r.json(); })
					.then(function(j){ setData(j); })
					.catch(function(){ setData(null); });
			};
			useEffect(function(){ load(); }, []);
			var setFallback = function(on){
				setBusy(true);
				fetch("/vision-config/free-channels",{method:"PUT", headers:{"content-type":"application/json", "x-vision-config-action":"free-channels"}, body: JSON.stringify({freeKeylessFallback: on===true})})
					.then(function(r){ return r.json().then(function(j){ return {ok:r.ok, body:j}; }); })
					.then(function(res){
						setBusy(false);
						if(!res.ok){ showToast((res.body && res.body.error) || "保存失败"); return; }
						setData(res.body);
						showToast(on===true ? "已开启免 Key 最终兜底" : "已关闭免 Key 最终兜底");
					})
					.catch(function(e){ setBusy(false); showToast(e && e.message ? String(e) : "保存失败"); });
			};
			var keylessChannels = (data && Array.isArray(data.channels) ? data.channels : []).filter(function(ch){ return ch && ch.keyless === true; });
			var keylessModels = [];
			for(var _ki=0; _ki<keylessChannels.length; _ki++){
				var _kc = keylessChannels[_ki];
				var _kms = Array.isArray(_kc.models) ? _kc.models : [];
				for(var _kj=0; _kj<_kms.length; _kj++) keylessModels.push((_kc.name||_kc.id)+" · "+_kms[_kj]);
			}
			var fallbackOn = data ? data.freeKeylessFallback === true : false;
			var blockCls = "vmo-free-block";
			var head = createElement("button",{type:"button", key:"head", className:"vmo-free-summary"+(open===true?" vmo-free-summary-open":""),
				"aria-expanded":open===true, onClick:function(){ setOpen(!open); }},
				createElement("span",{className:"vmo-free-summary-chevron", "aria-hidden":"true"}, IconChevronDownOutline14 !== null ? createElement(IconChevronDownOutline14,{size:12}) : "›"),
				createElement("span",{className:"vmo-free-summary-title"}, "免 Key 最终兜底"),
				createElement("span",{className:"vmo-free-summary-meta"}, fallbackOn ? "已开启 · 固定排在免费链路最后" : "已关闭"));
			var body;
			if(data === null){
				body = createElement("p",{className:"vmo-free-meta", key:"loading"}, "加载中…");
			} else if(open !== true){
				body = createElement("p",{className:"vmo-free-meta", key:"hint"}, "点击展开：免费链路全部失败后的最后防线（免 Key 匿名渠道，无需配置）。");
			} else {
				body = [
					createElement("p",{className:C.intro, key:"intro"},
						"识图瀑布第三层：上半区模型和上面的免费链路全部失败时，最后尝试免 Key 的匿名渠道（OVHcloud AI Endpoints，无需密钥，有频率限制）。固定排在整条链路的最末尾。"),
					createElement("label",{className:"vmo-free-toggleCard", key:"toggle"},
						createElement("input",{type:"checkbox", checked:fallbackOn, disabled:busy, onChange:function(){ setFallback(!fallbackOn); }}),
						createElement("span",null, "启用免 Key 最终兜底",
							createElement("span",{className:"vmo-free-toggle-hint"}, fallbackOn ? "已开启：作为整条识图链路的最后一步" : "已关闭：免费链路失败即结束，不再尝试免 Key 渠道"))),
					keylessModels.length>0
						? createElement("div",{key:"models", className:"vmo-free-chainBlock"},
							createElement("div",{className:"vmo-free-chainLabel"}, "兜底模型（按渠道顺序）："),
							createElement("div",{className:"vmo-free-chain"}, keylessModels.map(function(_km){
								return createElement("span",{key:_km, className:"vmo-free-chain-step"},
									createElement("span",{className:"vmo-free-chain-step-copy"}, _km));
							})))
						: createElement("p",{className:"vmo-free-meta", key:"nomodels"}, "当前没有免 Key 渠道。"),
					createElement("p",{key:"note", className:"vmo-free-meta"},
						"说明：这一层不需要任何配置；匿名额度有限，仅建议作为最后手段保留。")];
			}
			return createElement.apply(null, ["section",{className:blockCls, "aria-label":"免 Key 最终兜底"}, head, body]);
		};
		...
		var VisionSettingsSection = function(props) {
					...
					if(sys && Array.isArray(sys.systemGroups)){
						for(var gi=0; gi<sys.systemGroups.length; gi++){
							...
							for(var mi=0; mi<ms.length; mi++){
								...
								// 历史配置恢复组：条目自带真实 provider 与完整字段（baseUrl 等），恢复时不丢配置
								flat.push({
									provider: (typeof m.provider==="string"&&m.provider.length>0) ? m.provider : g.provider,
									model:m.id, name: typeof m.name==="string"?m.name:m.id,
									legacy: g.legacy===true,
									baseUrl: typeof m.baseUrl==="string"?m.baseUrl:"",
									displayName: typeof m.displayName==="string"?m.displayName:"",
									requestFormat: typeof m.requestFormat==="string"?m.requestFormat:"",
									reasoning: typeof m.reasoning==="string"?m.reasoning:"",
									visionOverride: typeof m.visionOverride==="string"?m.visionOverride:""
								});
							}
						}
					}
			...
			for(var _j=0; _j<systemModels.length; _j++){
				...
				// 历史配置恢复组不受忽略表过滤：用户删过的也能在这里找回
				if(_sm.legacy!==true && dismissed[_sm.provider+"/"+_sm.model]) continue;
			...
			var importOne = function(entry){
				...
				setBusy(true);
				fetch("/vision-config/vision-models",{method:"POST", headers:{"content-type":"application/json", "x-vision-config-action":"vision-models"}, body: JSON.stringify({provider: entry.provider, model: entry.model, name: entry.name, baseUrl: entry.baseUrl||"", displayName: entry.displayName||"", requestFormat: entry.requestFormat||"", reasoning: entry.reasoning||"", visionOverride: entry.visionOverride||""})})
				.then(function(r){ return r.json().then(function(j){ return {ok:r.ok, body:j}; }); })
				.then(function(res){
...
setModal({mode:"add", data:{id:"",provider:firstProvider,providerType:firstProvider,customProvider:"",model:"",name:"",description:"",baseUrl:"",requestFormat:"openai-completions"}});
			...
			sectionChildren.push(createElement(FreeKeylessModule, {key:"free-keyless", api: api, showToast: showToast}));
